...
t_create() {
  echo 'Creating topics for Orchestration'
...
q_create() {
  ...
  q_setup 'eks-saga-orchestrator' ${REGION_ID} ${ACCOUNT_ID}
}
...
s_create() {
  ...
  echo 'Setting up topic subscriptions for Orchestration'
  ...
  s_setup 'eks-saga-orchestrator' 'eks-saga-orders-success' ${REGION_ID} ${ACCOUNT_ID}
  s_setup 'eks-saga-orchestrator' 'eks-saga-inventory-fail' ${REGION_ID} ${ACCOUNT_ID}
}
