...
cleanUpOrchestration() {
  ...
  echo 'Performing clean-up of Orchestration demo'
...
cleanUp() {
  ...
  case "${DEMO_TYPE}" in
    O)
      cleanUpOrchestration ${REGION_ID} ${ACCOUNT_ID}
      ;;

    *)
      echo "Invalid value for demo type ${DEMO_TYPE}. Valid value is O(rchestration)."
      ;;
  esac
}
